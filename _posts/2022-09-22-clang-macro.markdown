...
> 注意虽然使用类函数宏时没有要求，但定义类函数宏时括号必须紧跟着宏名，不然会被识别成类对象宏。
{: .prompt-warning }
...
> 注意，字符串字面值中的参数不会被替换，考虑到记号化是在处理宏指令完成的，这并不令人感到意外。
{: .prompt-warning }
...
printf("i/j = %d\n", i/j);
```
> 注意，和后续会提到的宏的嵌套规则不同，当宏参数用于串化时，不会先进行宏展开，仅仅使用一遍串化是不能将宏的结果串化的。
{: .prompt-warning }
...
宏的替换列表可以包含对其他宏的调用，预处理器会重复检查替换列表，直到把所有宏名字替换掉为之。
...
可以看出，宏只是简单的替换操作，和真正的函数和对象其实质并不相同，因此必须通过括号来使其性质与对象和函数更为接近以减少误解。
